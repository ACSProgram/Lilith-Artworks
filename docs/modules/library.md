...
- `AppState` 按当前仓库路径缓存完整校验状态。首次访问、启动状态读取和设置保存执行 `integrity_check(1)`、`foreign_key_check`、format/version、迁移、实体 UUID、受控相对路径和持久化 SHA-256 格式校验；同一路径的后续命令只检查标准数据库仍存在且 format/schema version 仍为当前值。轻量检查失败会立即清除缓存，下一次访问必须重新完成完整校验。
- 所有仓库读写持有应用级仓库 lease；设置切换必须等待活动操作完成。前端保存新路径前立即清空旧仓库状态，并以 ready 仓库路径重建 Library/Artwork 工作区，因此仓库 A 与克隆仓库 B 即使包含相同 UUID，旧请求和选择也不会复用到 B。
...
- 永久删除和清空只能从回收站执行。应用工作流在共享运行锁和仓库 lease 内调用 Library 仓储，只把 Artwork 受控目录写入待清理队列，再提交元数据删除并运行清理；首次外部认证导出始终属于用户文件，不会由 Artwork 永久删除或清空回收站删除。仓库文件失败会返回清单并保留为可重试项。

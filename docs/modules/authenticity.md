...
质量预览和签名发布共享单一认证活动任务；前端可通过 `cancel_authenticity_operation` 请求取消，切换分支或离开发布页也会请求取消旧任务。图像打开、缩略图、背景合成、TrustMark、JPEG 编码、缓存发布、C2PA 签名、回读和摘要之间都检查取消；TrustMark 推理、单次图片编解码和 C2PA SDK 的本地调用无法在函数内部强制中断，因此取消延迟取决于当前阶段，并在下一边界生效。取消发生在缓存发布前时不留下缓存文件，发生在外部输出发布前时由临时目录自动清理；一旦清理意图已提交并开始原子发布，流程会完成数据库收尾或既有失败恢复，不在中间中断。

...
普通数据库连接只使用 SQLite `READ_WRITE` 打开现有 `lilith-artworks.sqlite3`，不得带 `CREATE`。仓库目录和新 schema 创建只从 Library 显式初始化入口进入；打开现有仓库仍负责格式、完整性、版本与迁移检查。已配置目录被外部删除或清空时，状态查询和后台线程只报告仓库不可用，不创建占位数据库。
